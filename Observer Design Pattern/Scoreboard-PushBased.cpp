...
class Observer{
    public:
        virtual void notify(CricketDataPacket)=0;
};
class Observable{
    public:
    ...
    public:
        ...
        void sendUpdate(){
            for(auto observer: observers){
                observer->notify(cricketDataPacket);
            }
        }
...
class Scoreboard: public Observer{
    ...
    public:
        Scoreboard(Observable &o){
            o.registerObserver(this);
        }
        void notify(CricketDataPacket c){
            cricketDataPacket.runs = c.runs;
            cricketDataPacket.overs = c.overs;
            cricketDataPacket.wickets = c.wickets;
            display();
        }
};
